...
def part1():
    ...
    for i in range(3):
        sigma1 = sigmas[i]
        sigma2 = sigmas[i+1]
        level = filters.gaussian(J, sigma=sigma1) - filters.gaussian(J, sigma=sigma2)
        DoG[:, :, i] = level
    ...
    local_minima = scipy.ndimage.filters.minimum_filter(DoG, size=22)
...
def is_4connected(row, col, row_id, col_id):
    ...
    """
    # Two pixels are 4-connected neighbors if they are either horizontally or vertically adjacent.
    return (abs(row_id - row) == 1 and col_id == col) or (abs(col_id - col) == 1 and row_id == row)
...
def getRegionalMinima(img):
    ...
    for row in range(h):
        for col in range(w):
            min_neighbor_row, min_neighbor_col = getSmallestNeighborIndex(img, row, col)
            if img[row, col] <= img[min_neighbor_row, min_neighbor_col]:
                if markers[row, col] == 0:
                    markers[row, col] = label_count
                    label_count += 1
...
def iterativeMinFollowing(img, markers):
    ...
    while True:
        ...
        for row in range(h):
            for col in range(w):
                if markers_copy[row, col] == 0:
                    min_neighbor_row, min_neighbor_col = getSmallestNeighborIndex(img, row, col)
                    if markers_copy[min_neighbor_row, min_neighbor_col] != 0:
                        markers_copy[row, col] = markers_copy[min_neighbor_row, min_neighbor_col]
                    else:
                        n_unmarked_pix += 1
        ...
        # Stop the iteration when all pixels are labeled
        if n_unmarked_pix == 0:
            break
